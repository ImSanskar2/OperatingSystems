...
int max(int pri[],int Pk[],int n)
{
	int i=0,max_i=Pk[0],max_v=pri[Pk[0]];
	for(i=1;i<=n;i++)
	{
		if(max_v<pri[Pk[i]])
		{
...
cin>>n;
int pro[n],At[n],Bt[n],Btemp[n],wt[n],Tat[n],pri[n];
wt[0]=0;
for(i =0;i<n;i++)
{
pro[i]=i;
cout<<"Enter Arrival,Burst Time And Priority Of Process "<<i<<endl;
cin>>At[i]>>Bt[i]>>pri[i];
Btemp[i]=Bt[i];
}
